"""
 ConvAE.py  (author: Shameer Sathar /  https://github.com/ssat335)
...
from ..utilities.image_utilities import load_icc_images_from_dir
from ..utilities.image_utilities import load_icc_images_from_dir_parallel
...
def main():
    ...
    # Set run parameters
    # =================================================
    training_dir = "db/icc_images_training"
    ...
    n_epochs = 20
    n_hidden_1 = 64  # 1st hidden layer
    n_hidden_2 = 32  # 2nd hidden layer
    n_hidden_3 = 32  # 3rd hidden layer
    ...
    if gray_scale:
        ...
        n_channels = 3
    x_train = normalize_reshape_img_data(x_train, n_channels)  # (n, ypixels, xpixels, n_channels)
    x_test = normalize_reshape_img_data(x_test, n_channels)  # (n, ypixels, xpixels, n_channels)
    ...
    if train_model:
        ...
        if 1:
            ...
            input_img = Input(shape=(ypixels, xpixels, n_channels))  # input layer (ypixels, xpixels, n_channels)
            ...
            x = UpSampling2D(poolkernel)(x)
            decoded = Conv2D(n_channels, convkernel, activation='sigmoid', padding='same')(x)  # output layer
        ...
        input_enc_shape = encoder.layers[0].input_shape[1:]  # (ypix, xpix, n_channels)
        ...
        output_dec_shape = decoder.layers[-1].output_shape[1:]  # (ypix, xpix, n_channels)
