"""
 image_utilities.py (author: Shameer Sathar /  https://github.com/ssat335)
...
class ImageUtils(object):
    ...
    """
     Load raw icc_images from a given directory, resize them, and save them
    """
    ...
    """
     Read a raw directory of icc_images, and load as numpy array to memory
    """
    def raw2resizednorm_load(self, raw_dir=None, img_shape=None):
        ...
        if img_list.shape[0] != n_files:
            raise Exception("Inconsistent number of loading icc_images!")
        if img_list.shape[1] != ypixels_force:
            raise Exception("Inconsistent ypixels loading icc_images!")
        if img_list.shape[2] != xpixels_force:
            raise Exception("Inconsistent xpixels loading icc_images!")
        if img_list.shape[3] != 3:
            raise Exception("Inconsistent RGB loading icc_images!")
    ...
    """
     Loads all icc_images from a given directory:
    ...
    """
    def load_icc_images(self, dir, ratio_training_test, seed):
        frac_training_use = 1
        gray_scale = False
    ...
    """
     Load icc_images from directory (parallelized to cores)
    """
    def load_icc_images_parallel(self, dir, ratio_training_test, seed, n_cores):
        frac_training_use = 1
        gray_scale = False
        index_train = []; index_test = []
        x_train = []; x_test = []
